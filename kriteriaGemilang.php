...
// Get awardID and kategoriGemilang from the URL or use session for awardID
...
// Get kategoriGemilang from the URL (if available)
if (isset($_GET['kategoriGemilang'])) {
    $kategoriGemilang = $_GET['kategoriGemilang'];
} else {
    $kategoriGemilang = ''; // Default value if not provided
}
...
// Fetch rubrik and segment data dynamically based on the awardID and kategoriGemilang
$query = "SELECT dr.rubrik, dr.segment, dr.requirementID, dr.keterangan
          FROM documentrequirements dr
          WHERE dr.awardID = ?";

// Add a condition to filter by kategoriGemilang if it is provided
if ($kategoriGemilang) {
    $query .= " AND dr.kategoriGemilang = ?"; 
}

$query .= " ORDER BY dr.rubrik, dr.segment";

$stmt = $conn->prepare($query);

// Bind parameters: awardID is always required, and kategoriGemilang is conditional
if ($kategoriGemilang) {
    $stmt->bind_param("is", $awardID, $kategoriGemilang); // 'i' for integer, 's' for string
} else {
    $stmt->bind_param("i", $awardID); // Only bind awardID if no kategoriGemilang filter
}
...
<body>
    ...
    <div class="container">
        <h2><?= htmlspecialchars($awardName) ?></h2>
        <p><b><?= htmlspecialchars($kategoriGemilang) ?></b></p>
        <table>
            <thead>
                <tr>
