...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hantar'])) {
    ...
    if ($updateStmt->affected_rows > 0) {
        ...
        header("Location: gemilangSubmission.php?awardID=$awardID&gradeID=$gradeID&kategoriGemilang=" . urlencode($_SESSION['kategoriGemilang']));
...
$segmentLabels = [
    ...
    // Add more segment types and their corresponding labels as needed
];
...
<body>
                    ...
                    <?php if (!empty($submissionsFromDB)): ?>
                        ...
                        <?php foreach ($submissionsFromDB as $submission): ?>
                            <tr>
                                ...
                                <td>
                                    <a href="?delete=<?php echo urlencode($submission['documentName']); ?>&requirementID=<?php echo urlencode($requirementID); ?>&awardID=<?php echo urlencode($awardID); ?>&gradeID=<?php echo urlencode($gradeID); ?>&kategoriGemilang=<?php echo urlencode($kategoriGemilang); ?>" class="btn btn-danger btn-sm">Buang</a>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-primary" onclick="confirmSubmission(<?= $awardID ?>, <?= $gradeID ?>, '<?= $kategoriGemilang?>')">Confirm</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        function confirmSubmission(awardID, gradeID, kategoriGemilang) {
        // Redirect to Gemilang_submission.php with all the parameters
        window.location.href = "gemilangSubmission.php?awardID=" + awardID + 
                            "&gradeID=" + gradeID + 
                            "&kategoriGemilang=" + encodeURIComponent(kategoriGemilang);
        }
        ...
        function handleDisabledClick(link) {
            if (link.classList.contains('disabled-link')) {
                return false; // Block click entirely
            }
